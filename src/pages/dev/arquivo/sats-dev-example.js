...
function App() {
  ...
  const [checkInscriptionNumber, setCheckInscriptionNumber] = useState();
  const [showClaimPage, setShowClaimPage] = useState(0);
  ...
  const unisat = typeof window !== "undefined" ? window.unisat : undefined;
    ...
    const typeText = (
      ...
      function type() {
        if (index < text.length) {
          ...
          setTyping1Finished(true);
        }
      }
      type();
    };

    typeText(text1, setDisplayedText1, setTyping1Finished);
  ...
  const runCapabilityCheck = async (walletType) => {
    if (walletType === 1) {
  ...
  const handleNetworkChanged = (uninetwork) => {
    setUniNetwork(uninetwork);
    getBasicInfo();
  };

  const getRealUpdateData = async (liveRealDataSSE) => {
